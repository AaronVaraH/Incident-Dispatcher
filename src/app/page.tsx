...
export default function IncidentDashboard() {
  ...
  const getChannelBadge = (ch: string) => {
    ...
    if (c === 'slack') {
      return (
        <span className="px-2 py-0.5 rounded bg-emerald-950/60 text-emerald-400 border border-emerald-800 text-xs font-mono uppercase">
          #slack
        </span>
      );
    }
    if (c === 'pagerduty') {
      return (
        <span className="px-2 py-0.5 rounded bg-violet-950/60 text-violet-300 border border-violet-800 text-xs font-mono uppercase">
          pagerduty
        </span>
      );
    }
    return (
      <span className="px-2 py-0.5 rounded bg-cyan-950/60 text-cyan-300 border border-cyan-800 text-xs font-mono uppercase">
        email-tls
      </span>
    );
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100 flex flex-col">
      ...
      <header className="border-b border-slate-800 bg-slate-900/60 backdrop-blur sticky top-0 z-30 px-6 py-4">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 bg-slate-800/90 border border-slate-700/80 px-3.5 py-1.5 rounded-lg text-xs font-mono shadow-sm">
              <Database className="w-3.5 h-3.5 text-cyan-400" />
              <span className="text-slate-400">Singleton Instance:</span>
              <span className="text-cyan-300 font-semibold">{sessionInstanceId}</span>
            </div>
      ...
      <div className="max-w-7xl mx-auto w-full px-6 pt-6">
        <div className="bg-slate-900/50 border border-slate-800/80 rounded-2xl p-4 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
            <div className="bg-slate-950/60 border border-slate-800/90 rounded-xl p-3 flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between text-indigo-400 font-mono font-medium mb-1">
                  <span>1. Strategy Pattern</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-indigo-500/10 border border-indigo-500/20">Behavioral</span>
                </div>
                <p className="text-slate-400 text-[11px] leading-relaxed">
                  Decouples SLA & header formatting via <code className="text-slate-200">Critical</code>, <code className="text-slate-200">Warning</code>, or <code className="text-slate-200">InfoEscalationStrategy</code>.
                </p>
              </div>
            ...
            <div className="bg-slate-950/60 border border-slate-800/90 rounded-xl p-3 flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between text-emerald-400 font-mono font-medium mb-1">
                  <span>2. Factory Pattern</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/20">Creational</span>
                </div>
                <p className="text-slate-400 text-[11px] leading-relaxed">
                  <code className="text-slate-200">DispatcherFactory</code> creates the proper channel client (<code className="text-slate-200">Slack</code>, <code className="text-slate-200">PagerDuty</code>, <code className="text-slate-200">Email</code>) without exposing concrete classes.
                </p>
              </div>
              <div className="mt-2 text-[10px] font-mono text-slate-500">
            ...
            <div className="bg-slate-950/60 border border-slate-800/90 rounded-xl p-3 flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between text-cyan-400 font-mono font-medium mb-1">
                  <span>3. Singleton Pattern</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-cyan-500/10 border border-cyan-500/20">Creational</span>
                </div>
                <p className="text-slate-400 text-[11px] leading-relaxed">
                  <code className="text-slate-200">AuditLogger.getInstance()</code> maintains a single in-memory store preserving <code className="text-slate-200">{sessionInstanceId.slice(0, 14)}...</code> across all requests.
                </p>
              </div>
              <div className="mt-2 text-[10px] font-mono text-slate-500">
      ...
      <main className="max-w-7xl mx-auto w-full p-6 grid grid-cols-1 lg:grid-cols-12 gap-6 flex-1">
        ...
        <section className="lg:col-span-7 flex flex-col gap-6">
            ...
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Submits payload to <code className="text-indigo-300 font-mono">/api/incident</code> route.
                </p>
              </div>
            ...
            <div className="flex flex-wrap gap-2 mb-5">
              {TEMPLATES.map((tmpl, idx) => (
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-slate-800/80 hover:bg-slate-700/80 border border-slate-700/80 text-[11px] text-slate-300 hover:text-white transition-colors"
                >
                  {tmpl.label}
                </button>
              ))}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1.5">
                    Target Channel <span className="text-slate-500">(Factory Pattern)</span>
                  </label>
                  <div className="relative">
                    <select
                      value={channel}
                      onChange={(e) => setChannel(e.target.value as typeof channel)}
                      className="w-full bg-slate-950 border border-slate-700 rounded-xl px-3.5 py-2.5 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
                    >
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1.5">
                    Severity Level <span className="text-slate-500">(Strategy Pattern)</span>
                  </label>
                  <div className="relative">
                    <select
                      value={severity}
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3.5 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all resize-none font-mono text-xs"
                />
              </div>
          ...
          {receipt ? (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl relative animate-in fade-in-50 duration-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4 text-xs">
                <div className="bg-slate-950/70 border border-slate-800 rounded-xl p-3">
                  ...
                  <p className="font-mono text-slate-300 break-all text-[11px]">
                ...
                <div className="bg-slate-950/70 border border-slate-800 rounded-xl p-3">
                  ...
                  <div className="flex items-center gap-2">
                    <code className="text-emerald-400 font-mono font-semibold text-[12px]">
              ...
              <div className="bg-slate-950/90 border border-slate-800 rounded-xl p-3.5 mb-4">
                ...
                <p className="font-mono text-xs text-slate-200 bg-slate-900/90 p-3 rounded-lg border border-slate-800 leading-relaxed break-words">
                  {receipt.payloadDelivered.formattedOutput}
                </p>
              </div>

              {/* Action Protocol */}
              <div className="text-xs text-slate-400 flex items-start gap-2 bg-slate-800/40 p-3 rounded-xl border border-slate-800">
                <Radio className="w-4 h-4 text-violet-400 shrink-0 mt-0.5" />
                <div>
                  <strong className="text-slate-300">Action Protocol: </strong>
              ...
              {pipelineTrace && (
                <div className="mt-4 pt-3 border-t border-slate-800/80 flex flex-wrap items-center justify-between gap-2 text-[11px] text-slate-400">
                  <div className="flex items-center gap-2">
                    ...
                    <span>Strategy: <code className="text-indigo-300">{pipelineTrace.strategy.strategyClass}</code></span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Server className="w-3.5 h-3.5 text-emerald-400" />
                    <span>Factory: <code className="text-emerald-300">{pipelineTrace.factory.dispatcherClass}</code></span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Database className="w-3.5 h-3.5 text-cyan-400" />
                    <span>Singleton Events: <code className="text-cyan-300">{pipelineTrace.singleton.totalLoggedEvents}</code></span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="bg-slate-900/40 border border-slate-800/60 rounded-2xl p-8 text-center text-slate-500">
              ...
              <p className="text-sm font-medium text-slate-400">Awaiting Alert Dispatch</p>
        ...
        <section className="lg:col-span-5 flex flex-col gap-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl flex-1 flex flex-col min-h-[500px]">
            {/* Header */}
            <div className="flex items-center justify-between border-b border-slate-800 pb-4 mb-4">
              <div>
                <div className="flex items-center gap-2">
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">
                  Managed globally by <code className="text-cyan-300 font-mono">AuditLogger.getInstance()</code>
                </p>
              </div>
              ...
              {logs.length > 0 && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-rose-950/40 border border-slate-700 hover:border-rose-800 text-xs text-slate-300 hover:text-rose-300 transition-colors"
            ...
            <div className="bg-slate-950/70 border border-cyan-900/40 rounded-xl p-3 mb-4 text-xs flex items-start gap-2.5">
              <span className="w-2 h-2 rounded-full bg-cyan-400 mt-1 shrink-0 animate-pulse"></span>
              <div className="leading-relaxed">
                <span className="text-slate-300 font-medium">Persistent Singleton Instance: </span>
                <code className="text-cyan-300 font-mono break-all">{sessionInstanceId}</code>
            ...
            <div className="space-y-3 overflow-y-auto max-h-[620px] pr-1 flex-1">
              {logs.length === 0 ? (
                ...
                logs.map((entry) => (
                  <div
                    key={entry.id}
                    className="bg-slate-950/80 border border-slate-800/90 rounded-xl p-3.5 text-xs hover:border-slate-700 transition-colors shadow-sm"
                  >
                    ...
                    <p className="font-mono text-[11px] text-slate-300 leading-relaxed break-words bg-slate-900/80 p-2.5 rounded-lg border border-slate-800/80">
                      {entry.message}
                    </p>
